...
def evaluate_prophet_model(
    ...
    if cross_validation_func is None:
        logger.warning(
            "Prophet cross validation not available; using in-sample predictions"
        )
        df_cv = prophet_df[['ds', 'y']].copy()
        preds = model.predict(df_cv[['ds']])
        df_cv = df_cv.merge(
